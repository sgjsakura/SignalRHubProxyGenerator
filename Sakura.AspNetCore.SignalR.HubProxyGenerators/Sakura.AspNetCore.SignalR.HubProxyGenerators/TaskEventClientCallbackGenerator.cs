...
{
	/// <inheritdoc />
	public override ClassDeclarationSyntax Generate(SemanticModel model, ClassDeclarationSyntax classDeclaration,
		Type hubClientType)
	{
	...
	private static MethodDeclarationSyntax GenerateBindingMethod(SemanticModel model,
		IEnumerable<StatementSyntax> bindingStatements)
	{
	...
	private static IEnumerable<StatementSyntax> GenerateCallbackMethodBindingStatement(SemanticModel model,
		string clientCallbackName, MethodDeclarationSyntax callbackMethod)
	{
	...
	private static ClassDeclarationSyntax GenerateEvent(SemanticModel model, ClassDeclarationSyntax classDeclaration,
		MethodInfo hubClientMethod, out MethodDeclarationSyntax delegateMethod)
	{
	...
	/// <returns>
	///     The corresponding delegate type which are compatible with <paramref name="method" />. the delegate type will
	///     always be a variant of either the System.Action or the System.Func series.
	/// </returns>
	private static TypeSyntax GetDelegateTypeForMethod(SemanticModel model, MethodInfo method)
	{
		...
		{
			if (paramList.Length == 0) return model.GetTypeSyntax<Action>();
			...
			return actionBaseType.MakeGenericType(model,
				paramList.Select(i => model.GetEquivalentType(i.ParameterType)));
		}
